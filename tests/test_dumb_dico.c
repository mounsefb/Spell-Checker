...
void charger_dictionnaire(const char *nomFichier, dict_t *dictionnaire) {
...
void rechercher_mots(const char *nomFichier, dict_t dictionnaire, dict_t *motsNonExistants) {
    FILE *fichier = fopen(nomFichier, "r");
    ...
    while (lecture(fichier, mot_verif)) {
        if (isupper(mot_verif[0]) == 0 && isalpha(mot_verif[0])) {
            res = dict_search(dictionnaire, mot_verif);

            if (res == NULL || res[0] == 0) {
                dict_add(*motsNonExistants, mot_verif);
                if (res == NULL) {
                    free(mot_verif);
            ...
            if (res != NULL && res[1] == 1) {
                ...
                if (res == NULL || res[0] == 0) {
                    ...
                    res = dict_search(dictionnaire, tmpExt);

                    if (res != NULL && res[0] == 0) {
                        dict_add(*motsNonExistants, tmpExt);
                    }
                }
...
void liberer_dict(dict_t *dict1, dict_t *dict2) {
    dict_delete(*dict1);
    dict_delete(*dict2);
}
...
void unitaire(int argc, char const *argv[]) {
    ...
    charger_dictionnaire(argv[1], &mydict);
    ...
    dict_t mots_non_existants = dict_new();
    rechercher_mots(argv[2], mydict, &mots_non_existants);
    end_recherche = clock();
    dict_printf_readable(mots_non_existants);
    double duree_recherche = ((double)(end_recherche - start_recherche)) / CLOCKS_PER_SEC;
    int nbre_mot_manquant = get_nbre_manquant(mots_non_existants);
    printf("Duree recherche = %lf sec\n", duree_recherche);
    printf("%d mots pas dans le dico\n", nbre_mot_manquant);
    printf("Liberation des dicts\n");
    liberer_dict(&mots_non_existants, &mydict);
}

void construction_courbe(int argc, char const *argv[]) {
    clock_t start_chargement, end_chargement, start_compression, end_compression, start_recherche, end_recherche;
    int nbwords = linecounter(argv[1]);
    FILE* ftemps=fopen("temps_dumb_dico.dat","w");
    double taille;
    // int k=nbwords;
    ...
    system("gnuplot -p -e \"f(x) = a * (x) + b ; fit f(x) 'temps_dumb_dico.dat' u 1:2 via a,b;"
                ...
                "plot 'temps_dumb_dico.dat' u 1:2 with line lt 4 axis x1y2 title 'Temps', a * (x) + b  with line lt 6 axis x1y2 title 'Fit Taille';"
                "g(x) = c * (x) + d ;fit g(x) 'temps_dumb_dico.dat' u 1:3 via c,d;"
                "replot 'temps_dumb_dico.dat' u 1:3 with line lt 2 title 'Recherche Arbre Lexicographique', g(x)  with line lt 8 title 'Fit Recherche'; quit\"");
}
